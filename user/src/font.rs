...
pub const FONT_FIRST_CHAR: u8 = 0x20; // Space
pub const FONT_LAST_CHAR: u8 = 0x7E;  // Tilde

/// Font data: 95 characters * 16 bytes each = 1520 bytes
/// Each byte represents one row of 8 pixels (MSB = leftmost)
pub static FONT_DATA: [u8; 1520] = [
    // 0x20: Space
...
/// Returns a slice of 16 bytes, each representing one row
#[inline]
pub fn get_char_bitmap(c: u8) -> &'static [u8] {
    if c < FONT_FIRST_CHAR || c > FONT_LAST_CHAR {
        // Return space for unprintable characters
        return &FONT_DATA[0..FONT_HEIGHT];
    }
    let index = (c - FONT_FIRST_CHAR) as usize * FONT_HEIGHT;
